...
===== Проброс HDFS для сохранения файлов при выключении контейнера

В домашнем каталоге должен лежать каталог `hdfs.....`
с необходимым содержиым (содержимое прилагается отдельно).
Туда мапится внутренний каталог HDFS контейнера.
