...
[source]
----
cat fs1_events_f49dba62-59fb-484c-8e25-abb90f16d2a0.txt 
�{"header":{"stageCreator":"HadoopFS_01","sourceId":"event:file-closed:1:1586448120032","stagesPath":"HadoopFS_01","trackingId":"event:file-closed:1:1586448120032::HadoopFS_01","previousTrackingId":null,"raw":null,"rawMimeType":null,"errorDataCollectorId":null,"errorPipelineName":null,"errorStage":null,"errorStageLabel":null,"errorCode":null,"errorMessage":null,"errorTimestamp":0,"errorStackTrace":null,"errorJobId":null,"values":{"sdc.event.version":"1","sdc.event.creation_timestamp":"1586448120032","sdc.event.type":"file-closed"}},"value":{"type":"MAP","value":{"filename":{"type":"STRING","value":"file_0ea69534-d866-4a91-b8fe-3d7b346eb335.json","dqpath":"/filename","sqpath":"/filename"},"filepath":{"type":"STRING","value":"/out/bronze/weather/file_0ea69534-d866-4a91-b8fe-3d7b346eb335.json","dqpath":"/filepath","sqpath":"/filepath"},"length":{"type":"LONG","value":"207079","dqpath":"/length","sqpath":"/length"}},"dqpath":"","sqpath":""}}
----
...
* Для стриминга в streamsets требуется bucketing, создал 2 бакета на основе поля times.
* Создана как external. +
  Для того, чтобы можно было сохранять/восстанавливать
  путем обычного копирования hfds - mapped local folder,
  при перезапуске контейнера (пробросить hdfs наружу из
  контейнера не вышло).
...
[source, bash]
----
su --command="hdfs dfs -put /media/shared_from_local/export/ /" hdfs

?
su --command="hdfs dfs -chmod -R 777 /export" hdfs
----
...
----
use bronze;
import table tsource from '/export/bronze/tsource';
----

[source, sql]
----
-- If table was not previously imported
create table
  tsource (
    text string,
    times bigint,
  ...
  partitioned by (keypartition string)
  clustered by (times) into 2 buckets
  stored as orc
;
----
...
* Start pipeline BronzeLayerStream
...
----
use bronze;
export table tsource to '/export/bronze/tsource/';
----
...
[source, bash]
----
rm -f -R /media/shared_from_local/export/* && su  --command="hdfs dfs -get /export/* /media/shared_from_local/export/" hdfs
----
