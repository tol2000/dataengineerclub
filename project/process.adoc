...
По доке по апи стриминга хайва партишин поля должны идти в конце
For example, if table is partitioned by 2 columns (year and month), hive streaming connection will extract last 2 columns from the input records using which partitions will be created dynamically in metastore. 
https://cwiki.apache.org/confluence/display/Hive/Streaming+Data+Ingest+V2
А у меня при автосоздании партиции из стримсета создается год 4 (месяц) вместо 2020
...
--  tblproperties ('transactional'='true')
  ...
  clustered by (keybucketing) into 2 buckets
  stored as orc
;
----
