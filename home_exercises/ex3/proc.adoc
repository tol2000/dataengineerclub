...
* С докером не хотел завязываться, хотел настроить по-взрослому, т.к. потом может пригодиться этот опыт.
* Развернул в GCP три машины на центосе 7

=== Попытка 1

* Попытался сделать систему из трех машин: одна менеджер и две в кластере.
* Не получилось, т.к. в гугле ограничения на ресурсы, и если с процессором
  и памятью я ужался, то для веб-интерфейса hue нужен был стат. ip,
  а гугл позволяет только один в регионе (этот один уже у клаудера менеджера)

=== Попытка 2

* Сделал кластер из одной машины, на этой же машине и клаудера менеджер
* В реале это, конечно, утопичная система, но для теста, думаю, подойдет :) +
  ----
...
* В итоге файл загрузился и я создал внешнюю таблицу, которая на этот файл ссылается. +
  ...
  ----
* В качестве факультатива выполнил сложнейший запрос к таблице и посмотрел, кто там числится в списке атлетов :) +
  ----
